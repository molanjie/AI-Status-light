...
type NetEntry = Awaited<ReturnType<typeof si.networkStats>>[number]

function pickDisk(fsList: FsEntry[]): FsEntry | null {
  if (process.platform === 'darwin') {
    return fsList.find(d => d.mount === '/System/Volumes/Data')
  ...
  if (process.platform === 'win32') {
    const systemDrive = (process.env.SystemDrive ?? os.homedir().slice(0, 2)).toUpperCase()
    return fsList.find(d => d.mount?.toUpperCase().startsWith(systemDrive))
      ?? fsList.find(d => d.fs?.toUpperCase().startsWith(systemDrive))
      ?? fsList[0]
      ?? null
  }

  return fsList.find(d => d.mount === '/') ?? fsList[0] ?? null
}

function pickNetwork(netList: NetEntry[]): NetEntry | null {
  const isLoopback = (iface?: string) => {
    const name = (iface ?? '').toLowerCase()
    return name === 'lo' || name === 'lo0' || name.includes('loopback')
  }
  return netList.find(n => !isLoopback(n.iface) && ((n.rx_sec ?? 0) > 0 || (n.tx_sec ?? 0) > 0))
    ?? netList.find(n => !isLoopback(n.iface))
    ?? netList[0]
    ?? null
}
...
export async function collectSystemStats(): Promise<SystemStats> {
  ...
  // macOS APFS uses /System/Volumes/Data for user data; Windows exposes drive letters.
  const root = pickDisk(fsList)
  const net = pickNetwork(netList)
