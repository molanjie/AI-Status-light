...
async function windowsModel(): Promise<string | null> {
  try {
    const command = 'powershell -NoProfile -ExecutionPolicy Bypass -Command "(Get-CimInstance Win32_ComputerSystem).Model"'
...
async function machineModel(platform: NodeJS.Platform): Promise<string | null> {
  switch (platform) {
    case 'darwin': return macModel()
    case 'win32': return windowsModel()
    default: return null
  }
}
...
async function _doCollect(): Promise<BaseInfo> {
  ...
  const info: BaseInfo = {
    ...
    model: await machineModel(platform),
