...
function serveProjectPublicHtml(fileName: string) {
  return async (c: any) => {
    const filePath = path.join(projectPublicPath, fileName)
    if (!existsSync(filePath)) return c.notFound()
    return c.html(readFileSync(filePath, 'utf-8'), 200, {
...
export function setupStatic(app: Hono<AppBindings>) {
  app.get('/', c => c.redirect('/phone', 302))
  app.get('/phone', serveProjectPublicHtml('traffic-light.html'))
  app.get('/light', serveProjectPublicHtml('traffic-light.html'))
  app.get('/traffic-light', serveProjectPublicHtml('traffic-light.html'))
  app.get('/traffic-light.html', serveProjectPublicHtml('traffic-light.html'))
  app.get('/traffic-control.html', serveProjectPublicHtml('traffic-control.html'))
  app.get('/dynamic-island.html', serveProjectPublicHtml('dynamic-island.html'))
