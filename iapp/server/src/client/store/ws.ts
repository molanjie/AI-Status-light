...
import type { PageInfo, WsStatus, SessionSummary, SessionCounts, SignalLightState } from '@client/types/ws'
...
export const useWsStore = defineStore('ws', () => {
  ...
  // ─── 红绿灯信号 ───
  const signalLight = ref<SignalLightState>({
    aggregate: 'idle',
    signal: 'idle',
    session_id: '',
    session_summaries: [],
    session_counts: { total: 0, working: 0, permission: 0, error: 0, attention: 0, idle: 0 },
    ts: 0,
  })
    ...
    ws.onmessage = (e) => {
      ...
      try {
        ...
        switch (msg.type) {
          ...
          case 'signal_light':
            signalLight.value = {
              aggregate: msg.aggregate ?? 'idle',
              signal: msg.signal ?? msg.aggregate ?? 'idle',
              session_id: msg.session_id ?? '',
              session_summaries: msg.session_summaries ?? [],
              session_counts: msg.session_counts ?? { total: 0, working: 0, permission: 0, error: 0, attention: 0, idle: 0 },
              ts: msg.ts ?? 0,
            }
            break
        }
