...
export default defineConfig({
  ...
  plugins: [
    ...
    {
      name: 'mini-watch-traffic-static',
      configureServer(server) {
        server.middlewares.use((req, res, next) => {
          const pathname = req.url?.split('?')[0] ?? ''
          if (pathname === '/traffic-light.html' || pathname === '/traffic-control.html' || pathname === '/dynamic-island.html') {
            const file = pathname.slice(1)
            res.statusCode = 200
            res.setHeader('content-type', 'text/html; charset=utf-8')
            res.end(readFileSync(resolve(__dirname, 'public', file), 'utf-8'))
            return
          }
          next()
        })
